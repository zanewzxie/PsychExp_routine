...
fileName = [filedirectoy 'data/' subjectNumber '_' sessionNumber '_CWMC.txt'];
dataFile = fopen(fileName, 'a');
startTime = datestr(now, 0);
if (strcmp(prac,'N') || strcmp(prac,'n'))
    fprintf(dataFile, '%s\t', startTime);
    fprintf(dataFile, '%i\n', subjectNumber);
    fprintf(dataFile, 'Subject\tTrial\tChange\tClickX\tClickY\tReportedChange\tAccuracy\tRT\n');
end
...
objectSize=centerX*0.06;
offsetSize=centerX*0.12;
...
if strcmp(prac,'Y')
    ...
    DrawFormattedText(expWin,'press any mouse key to start', 'center', 'center', [0 0 0], 90);
    Screen('Flip',expWin);
else
    Screen('FillRect', expWin, backgroundCol);
    DrawFormattedText(expWin,'press any mouse key to start','center', 'center');
    Screen('Flip',expWin);
end
...
for tt=1:numTrials
    ...
    if ((strcmp(prac,'N') || strcmp(prac,'n')) && tt==numTrials/2+1)
        Screen('FillRect', expWin, backgroundCol);
        DrawFormattedText(expWin,  'press any key on the mouse to start ', 'center', 'center', [0 0 0], 90);
        Screen('Flip',expWin);
        clickstart=0;
        while clickstart==0 %&& ((GetSecs-startResp)<longestResp)
            [clicks, mx, my, whichButton]=GetClicks(expWin, .5);
...
%         [mx,my,mbuttons] = GetMouse(expWin);
    ...
    %header='Subject\tTrial\tCueLocations\tCueColorCodes\tChange\tClickX\tClickY\tReportedChange\tAccuracy\tRT\n
                fprintf(dataFile, '%s\t%d\t%d\t%d\t%d\t%d\t%d\t%f\n',...
                    subjectNumber, tt, change(tt), clickloc(tt,1), clickloc(tt,2), find(changereport(tt,:)), accuracy(tt), clickTime(tt));
    end
